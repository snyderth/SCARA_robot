...
// ============================================================
// File Name: DoubleToFixed42Bit.v
// Megafunction Name(s):
// 			ALTFP_CONVERT
//
...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" OPERATION="FLOAT2FIXED" ROUNDING="TO_NEAREST" WIDTH_DATA=64 WIDTH_EXP_INPUT=11 WIDTH_EXP_OUTPUT=8 WIDTH_INT=32 WIDTH_MAN_INPUT=52 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=47 clk_en clock dataa nan overflow result underflow
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" PIPELINE=2 SHIFTDIR="VARIABLE" SHIFTTYPE="LOGICAL" WIDTH=98 WIDTHDIST=7 aclr clk_en clock data direction distance result
//VERSION_BEGIN 18.1 cbx_altbarrel_shift 2018:09:12:13:04:24:SJ cbx_mgl 2018:09:12:13:10:36:SJ  VERSION_END

//synthesis_resources = reg 201 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  DoubleToFixed42Bit_altbarrel_shift_p4h
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[97:0]	sbit_piper1d;
	reg	[97:0]	sbit_piper2d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	reg	sel_pipec6r1d;
	wire  [7:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 98'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[391:294];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 98'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[685:588];
	// synopsys translate_off
	initial
		sel_pipec4r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		pad_w = {64{1'b0}},
		result = sbit_w[783:686],
		sbit_w = {sbit_piper2d, smux_w[587:392], sbit_piper1d, smux_w[293:0], data},
		sel_w = {sel_pipec6r1d, sel_pipec5r1d, sel_pipec4r1d, distance[3:0]},
		smux_w = {((({98{(sel_w[6] & (~ dir_w[6]))}} & {sbit_w[621:588], pad_w[63:0]}) | ({98{(sel_w[6] & dir_w[6])}} & {pad_w[63:0], sbit_w[685:652]})) | ({98{(~ sel_w[6])}} & sbit_w[685:588])), ((({98{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[555:490], pad_w[31:0]}) | ({98{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[587:522]})) | ({98{(~ sel_w[5])}} & sbit_w[587:490])), ((({98{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[473:392], pad_w[15:0]}) | ({98{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[489:408]})) | ({98{(~ sel_w[4])}} & sbit_w[489:392])), ((({98{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[383:294], pad_w[7:0]}) | ({98{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[391:302]})) | ({98{(~ sel_w[3])}} & sbit_w[391:294])), ((({98{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[289:196], pad_w[3:0]}) | ({98{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[293:200]})) | ({98{(~ sel_w[2])}} & sbit_w[293:196])), ((({98{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[193:98], pad_w[1:0]}) | ({98{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[195:100]})) | ({98{(~ sel_w[1])}} & sbit_w[195:98])), ((({98{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[96:0], pad_w[0]}) | ({98{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[97:1]})) | ({98{(~ sel_w[0])}} & sbit_w[97:0]))};
endmodule //DoubleToFixed42Bit_altbarrel_shift_p4h

//synthesis_resources = lpm_add_sub 6 lpm_compare 4 reg 471 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  DoubleToFixed42Bit_altfp_convert_mtp
	( 
	clk_en,
	clock,
	dataa,
...
`endif
	...
	wire  [97:0]   wire_altbarrel_shift2_result;
	...
	reg	int_or_reg2;
	reg	int_or_reg3;
	reg	[46:0]	integer_result_reg;
	reg	[45:0]	integer_rounded_reg;
	...
	wire  wire_below_lower_limit2_alb;
	...
	wire  add_1_cout_w;
	wire  add_1_w;
	wire  [46:0]  all_zeroes_w;
	wire  barrel_direction_negative;
	wire  [97:0]  barrel_mantissa_input;
	wire  [44:0]  barrel_zero_padding_w;
	wire  below_limit_exceeders;
	wire  [46:0]  below_limit_integer;
	...
	wire  [46:0]  exceed_limit_integer;
	...
	wire  [46:0]  infinity_value_w;
	wire  int_or1_w;
	wire  int_or2_w;
	wire  [46:0]  integer_output;
	wire  [45:0]  integer_post_round;
	wire  [45:0]  integer_pre_round;
	wire  [46:0]  integer_result;
	wire  [45:0]  integer_rounded;
	wire  [45:0]  integer_rounded_tmp;
	wire  [45:0]  integer_tmp_output;
	wire  [22:0]  inv_add_1_adder1_w;
	wire  [22:0]  inv_add_1_adder2_w;
	wire  [45:0]  inv_integer;
	wire  [97:0]  lbarrel_shift_result_w;
	wire  [45:0]  lbarrel_shift_w;
	wire  lower_limit_selector;
	wire  lowest_integer_selector;
	wire  [45:0]  lowest_integer_value;
	wire  [25:0]  man_bus1;
	wire  [25:0]  man_bus2;
	wire  [25:0]  man_or1;
	wire  man_or1_w;
	...
	wire  nan_input_w;
	wire  [46:0]  neg_infi_w;
	wire  [10:0]  padded_exponent_input;
	wire  [46:0]  pos_infi_w;
	...
	wire  sign_input;
	wire  sign_input_w;
	wire  [45:0]  signed_integer;
	wire  sticky_bit_w;
	wire  [50:0]  sticky_bus;
	wire  [50:0]  sticky_or;
	wire  [45:0]  unsigned_integer;
	wire  upper_limit_w;
	wire  zero_input_w;

	DoubleToFixed42Bit_altbarrel_shift_p4h   altbarrel_shift2
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_result_reg <= 47'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_rounded_reg <= 46'b0;
	...
	.datab(46'b0000000000000000000000000000000000000000000001),
	.overflow(),
	...
	defparam
		...
		below_lower_limit1.lpm_width = 11,
	...
	defparam
		...
		below_lower_limit2.lpm_width = 11,
	...
	defparam
		...
		exceed_upper_limit.lpm_width = 11,
	...
	assign
		...
		barrel_zero_padding_w = {45{1'b0}},
		below_limit_exceeders = ((((denormal_input_w | zero_input_w) | nan_input_w) | below_lower_limit3_reg4) & (~ border_lower_limit_reg4)),
		below_limit_integer = (({47{(~ below_limit_exceeders)}} & integer_output) | ({47{below_limit_exceeders}} & all_zeroes_w)),
		...
		exceed_limit_integer = (({47{(~ exceed_limit_exceeders)}} & below_limit_integer) | ({47{exceed_limit_exceeders}} & infinity_value_w)),
		...
		infinity_value_w = (({47{(~ sign_input_w)}} & pos_infi_w) | ({47{sign_input_w}} & neg_infi_w)),
		int_or1_w = man_or2[0],
		int_or2_w = man_or1[6],
		...
		integer_rounded = (({46{(~ lowest_integer_selector)}} & integer_rounded_tmp) | ({46{lowest_integer_selector}} & lowest_integer_value)),
		integer_rounded_tmp = (({46{(~ add_1_w)}} & integer_pre_round) | ({46{add_1_w}} & integer_post_round)),
		integer_tmp_output = (({46{(~ sign_input_w)}} & unsigned_integer) | ({46{sign_input_w}} & signed_integer)),
		inv_add_1_adder1_w = wire_add_sub3_result,
		inv_add_1_adder2_w = (({23{(~ wire_add_sub3_cout)}} & inv_integer[45:23]) | ({23{wire_add_sub3_cout}} & wire_add_sub4_result)),
		inv_integer = (~ integer_rounded_reg),
		lbarrel_shift_result_w = wire_altbarrel_shift2_result,
		lbarrel_shift_w = lbarrel_shift_result_w[97:52],
		lower_limit_selector = ((((below_lower_limit3_reg4 & (~ zero_input_w)) & (~ denormal_input_w)) & (~ lowest_int_sel_reg)) & (below_lower_limit2_reg4 & (~ border_lower_limit_reg4))),
		...
		neg_infi_w = {1'b1, {46{1'b0}}},
		overflow = overflow_reg,
		padded_exponent_input = exponent_input,
		pos_infi_w = {1'b0, {46{1'b1}}},
...
endmodule //DoubleToFixed42Bit_altfp_convert_mtp
//VALID FILE
...
module DoubleToFixed42Bit (
	...
	wire [46:0] result = sub_wire2[46:0];
	wire  underflow = sub_wire3;

	DoubleToFixed42Bit_altfp_convert_mtp	DoubleToFixed42Bit_altfp_convert_mtp_component (
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
				.nan (sub_wire0),
...
// Retrieval info: CONSTANT: WIDTH_RESULT NUMERIC "47"
...
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit.v TRUE FALSE
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit.qip TRUE FALSE
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit.bsf FALSE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit_inst.v TRUE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit_bb.v FALSE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit.inc FALSE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL DoubleToFixed42Bit.cmp FALSE TRUE
