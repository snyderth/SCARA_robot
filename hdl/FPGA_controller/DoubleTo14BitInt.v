...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" OPERATION="FLOAT2INT" ROUNDING="TO_NEAREST" WIDTH_DATA=64 WIDTH_EXP_INPUT=11 WIDTH_EXP_OUTPUT=8 WIDTH_INT=14 WIDTH_MAN_INPUT=52 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=14 clk_en clock dataa overflow result underflow
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" PIPELINE=2 SHIFTDIR="LEFT" SHIFTTYPE="LOGICAL" WIDTH=65 WIDTHDIST=7 aclr clk_en clock data distance result
//VERSION_BEGIN 18.1 cbx_altbarrel_shift 2018:09:12:13:04:24:SJ cbx_mgl 2018:09:12:13:10:36:SJ  VERSION_END

//synthesis_resources = reg 135 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  DoubleTo14BitInt_altbarrel_shift_nof
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[64:0]	sbit_piper1d;
	reg	[64:0]	sbit_piper2d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	reg	sel_pipec6r1d;
	wire  [7:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 65'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[259:195];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 65'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[454:390];
	// synopsys translate_off
	initial
		sel_pipec4r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		direction_w = 1'b0,
		pad_w = {64{1'b0}},
		result = sbit_w[519:455],
		sbit_w = {sbit_piper2d, smux_w[389:260], sbit_piper1d, smux_w[194:0], data},
		sel_w = {sel_pipec6r1d, sel_pipec5r1d, sel_pipec4r1d, distance[3:0]},
		smux_w = {((({65{(sel_w[6] & (~ dir_w[6]))}} & {sbit_w[390], pad_w[63:0]}) | ({65{(sel_w[6] & dir_w[6])}} & {pad_w[63:0], sbit_w[454]})) | ({65{(~ sel_w[6])}} & sbit_w[454:390])), ((({65{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[357:325], pad_w[31:0]}) | ({65{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[389:357]})) | ({65{(~ sel_w[5])}} & sbit_w[389:325])), ((({65{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[308:260], pad_w[15:0]}) | ({65{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[324:276]})) | ({65{(~ sel_w[4])}} & sbit_w[324:260])), ((({65{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[251:195], pad_w[7:0]}) | ({65{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[259:203]})) | ({65{(~ sel_w[3])}} & sbit_w[259:195])), ((({65{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[190:130], pad_w[3:0]}) | ({65{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[194:134]})) | ({65{(~ sel_w[2])}} & sbit_w[194:130])), ((({65{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[127:65], pad_w[1:0]}) | ({65{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[129:67]})) | ({65{(~ sel_w[1])}} & sbit_w[129:65])), ((({65{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[63:0], pad_w[0]}) | ({65{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[64:1]})) | ({65{(~ sel_w[0])}} & sbit_w[64:0]))};
endmodule //DoubleTo14BitInt_altbarrel_shift_nof

//synthesis_resources = lpm_add_sub 5 lpm_compare 4 reg 331 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  DoubleTo14BitInt_altfp_convert_ecp
	( 
	clk_en,
	clock,
	dataa,
...
`endif
	...
	wire  [64:0]   wire_altbarrel_shift6_result;
	...
	reg	int_or_reg2;
	reg	int_or_reg3;
	reg	[13:0]	integer_result_reg;
	reg	[12:0]	integer_rounded_reg;
	...
	wire  add_1_cout_w;
	wire  add_1_w;
	wire  [13:0]  all_zeroes_w;
	wire  [64:0]  barrel_mantissa_input;
	wire  [11:0]  barrel_zero_padding_w;
	wire  below_limit_exceeders;
	wire  [13:0]  below_limit_integer;
	...
	wire  [13:0]  exceed_limit_integer;
	...
	wire  [13:0]  infinity_value_w;
	wire  int_or1_w;
	wire  [13:0]  integer_output;
	wire  [12:0]  integer_post_round;
	wire  [12:0]  integer_pre_round;
	wire  [13:0]  integer_result;
	wire  [12:0]  integer_rounded;
	wire  [12:0]  integer_rounded_tmp;
	wire  [12:0]  integer_tmp_output;
	wire  [5:0]  inv_add_1_adder1_w;
	wire  [6:0]  inv_add_1_adder2_w;
	wire  [12:0]  inv_integer;
	wire  [64:0]  lbarrel_shift_result_w;
	wire  [12:0]  lbarrel_shift_w;
	wire  lower_limit_selector;
	wire  lowest_integer_selector;
	wire  [12:0]  lowest_integer_value;
	wire  [25:0]  man_bus1;
	wire  [25:0]  man_bus2;
	wire  [25:0]  man_or1;
	wire  man_or1_w;
	...
	wire  nan_input_w;
	wire  [13:0]  neg_infi_w;
	wire  [10:0]  padded_exponent_input;
	wire  [13:0]  pos_infi_w;
	wire  [6:0]  power2_value_w;
	wire  [13:0]  result_w;
	wire  round_bit_w;
	wire  sign_input;
	wire  sign_input_w;
	wire  [12:0]  signed_integer;
	wire  sticky_bit_w;
	wire  [50:0]  sticky_bus;
	wire  [50:0]  sticky_or;
	wire  [12:0]  unsigned_integer;
	wire  upper_limit_w;
	wire  zero_input_w;

	DoubleTo14BitInt_altbarrel_shift_nof   altbarrel_shift6
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_result_reg <= 14'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_rounded_reg <= 13'b0;
	...
	assign
		...
		all_zeroes_w = {1'b0, {13{1'b0}}},
		barrel_mantissa_input = {barrel_zero_padding_w, implied_mantissa_input},
		barrel_zero_padding_w = {12{1'b0}},
		below_limit_exceeders = (((denormal_input_w | zero_input_w) | lower_limit_selector) | nan_input_w),
		below_limit_integer = (({14{(~ below_limit_exceeders)}} & integer_output) | ({14{below_limit_exceeders}} & all_zeroes_w)),
		...
		const_bias_value_add_width_res_w = 11'b10000001100,
		...
		exceed_limit_integer = (({14{(~ exceed_limit_exceeders)}} & below_limit_integer) | ({14{exceed_limit_exceeders}} & infinity_value_w)),
		...
		infinity_value_w = (({14{(~ sign_input_w)}} & pos_infi_w) | ({14{sign_input_w}} & neg_infi_w)),
		int_or1_w = man_or2[13],
		...
		integer_rounded = (({13{(~ lowest_integer_selector)}} & integer_rounded_tmp) | ({13{lowest_integer_selector}} & lowest_integer_value)),
		integer_rounded_tmp = (({13{(~ add_1_w)}} & integer_pre_round) | ({13{add_1_w}} & integer_post_round)),
		integer_tmp_output = (({13{(~ sign_input_w)}} & unsigned_integer) | ({13{sign_input_w}} & signed_integer)),
		inv_add_1_adder1_w = wire_add_sub8_result,
		inv_add_1_adder2_w = (({7{(~ wire_add_sub8_cout)}} & inv_integer[12:6]) | ({7{wire_add_sub8_cout}} & wire_add_sub9_result)),
		inv_integer = (~ integer_rounded_reg),
		lbarrel_shift_result_w = wire_altbarrel_shift6_result,
		lbarrel_shift_w = lbarrel_shift_result_w[64:52],
...
endmodule //DoubleTo14BitInt_altfp_convert_ecp
//VALID FILE
...
module DoubleTo14BitInt (
	...
	DoubleTo14BitInt_altfp_convert_ecp	DoubleTo14BitInt_altfp_convert_ecp_component (
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
				.overflow (sub_wire0),
